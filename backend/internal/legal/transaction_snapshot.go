...
func (s TransactionSnapshot) Validate() error {
	values:=[]string{s.SellerOrServiceProviderID,s.CommercialOwnerID,s.PaymentRecipientID,s.PlatformRole,s.FiscalResponsibilityID,s.RefundResponsibilityID,s.PayoutBeneficiaryID,s.PolicyVersion}
	for _,value:=range values { if strings.TrimSpace(value)=="" { return ErrIncompleteTransactionSnapshot } }
	return nil
}
