...
store_lifecycle_event_append_only_count="$(
  psql --dbname="$RESTORE_DATABASE" -Atc     "SELECT count(*) FROM pg_trigger WHERE tgname = 'store_subscription_lifecycle_events_append_only' AND NOT tgisinternal;"
)"
store_projection_guard_count="$(
  psql --dbname="$RESTORE_DATABASE" -Atc     "SELECT count(*) FROM pg_trigger WHERE tgname = 'store_subscription_projections_update_guard' AND NOT tgisinternal;"
)"
store_lifecycle_effect_append_only_count="$(
  psql --dbname="$RESTORE_DATABASE" -Atc     "SELECT count(*) FROM pg_trigger WHERE tgname = 'store_lifecycle_effects_append_only' AND NOT tgisinternal;"
)"
store_lifecycle_apply_function_count="$(
  psql --dbname="$RESTORE_DATABASE" -Atc     "SELECT count(*) FROM pg_proc WHERE proname = 'apgic_apply_store_lifecycle_event';"
)"
...
cat >"$evidence_dir/restore-drill.json" <<JSON
{
  ...
  "integrity_probe_store_lifecycle_event_append_only_count": $store_lifecycle_event_append_only_count,
  "integrity_probe_store_projection_guard_count": $store_projection_guard_count,
  "integrity_probe_store_lifecycle_effect_append_only_count": $store_lifecycle_effect_append_only_count,
  "integrity_probe_store_lifecycle_apply_function_count": $store_lifecycle_apply_function_count,
